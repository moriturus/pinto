...
fn collect_markdown_files(directory: &Path, files: &mut Vec<String>) {
    for entry in fs::read_dir(directory).expect("read documentation directory") {
        ...
        if path.is_dir() {
            ...
            files.push(fs::read_to_string(path).expect("read documentation file"));
        }
    }
}

#[test]
fn contributor_guidance_uses_agents_and_matches_quality_gate() {
    let agents = repository_file("AGENTS.md");
    assert!(
        !contains_japanese(&agents),
        "AGENTS.md must use English for all natural-language guidance"
    );
    for heading in [
        "# Overview",
        "## Project Overview",
        ...
        assert!(agents.contains(heading), "AGENTS.md omits {heading}");
    }
    for command in [
        "mise install",
        "mise run test",
        ...
        assert!(agents.contains(command), "AGENTS.md omits {command}");
    }
    assert!(agents.contains("test + lint + Rust/mdBook docs + fmt --check"));

    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    for directory in ["docs", ".github"] {
        let mut documents = Vec::new();
        collect_markdown_files(&root.join(directory), &mut documents);
        for document in documents {
            assert!(
                !document.contains("CLAUDE.md"),
                "documentation must refer to AGENTS.md, not CLAUDE.md"
            );
        }
    }
}
