...
fn edit_sets_parent_and_show_reports_both_directions() {
    ...
    // Set T-1 (Epic) as the parent of T-2 (Story).
    pinto(dir.path())
    ...
    // The parent is persisted in the frontmatter.
    let task = std::fs::read_to_string(dir.path().join(".pinto/tasks/T-2.md")).expect("task file");
    assert!(task.contains("parent = \"T-1\""));

    // The child shows Parent and the parent shows Children, confirming both directions.
    pinto(dir.path())
        .args(["show", "T-2"])
        .assert()
        .success()
...
fn edit_parent_cycle_exits_with_code_1() {
    ...
    // A ← B (B's parent is A); setting A's parent to B creates a cycle and fails.
    pinto(dir.path())
        .args(["edit", "T-2", "--parent", "T-1"])
        .assert()
        .success();
...
fn dep_add_sets_dependency_and_show_reports_both_directions() {
    ...
    // T-2 (Feature) depends on T-1 (Foundation).
    pinto(dir.path())
        .args(["dep", "add", "T-2", "T-1"])
        .assert()
        .success()
...
fn dep_add_cycle_warns_but_succeeds() {
    let dir = TempDir::new().expect("temp dir");
    init_with_items(dir.path(), &["A", "B"]);
    // B depends on A; adding a dependency from A to B creates a cycle (warning, but success).
    pinto(dir.path())
        .args(["dep", "add", "T-2", "T-1"])
        .assert()
        .success();
    ...
    // The relationship is still recorded despite the cycle.
...
fn link_add_records_commits_and_show_lists_them() {
    ...
    // Manual links are stored as plain strings even in a directory without Git.
    pinto(dir.path())
    ...
    // --json contains full SHAs in an array (the machine-readable contract).
...
fn link_rm_unlinks_by_prefix() {
    ...
    // A shortened SHA can be removed by prefix match.
    pinto(dir.path())
...
fn link_sync_associates_commits_from_messages() {
    ...
    // Each PBI receives exactly one link.
    ...
    // A rescan does not add duplicates (the operation is idempotent).
    pinto(dir.path())
        .args(["link", "sync"])
        .assert()
        .success()
