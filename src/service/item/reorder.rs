...
///
/// A rank is generated by [`Rank::rebalance`] when its scope has grown beyond
/// the shortest fixed width for that number of siblings. The existing order
/// within each scope is retained, while other scopes keep both their ranks and
/// timestamps. If `dry_run` is true, only the planned statistics are returned.
///
