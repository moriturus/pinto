...
///
/// A few strings come from `clap` itself and are intentionally left in English (out of scope):
/// the `Usage:` prefix, the auto-generated `-h`/`--help` and `-V`/`--version` flag descriptions,
/// the generated `help` subcommand, and the `Possible values:` block `clap` renders from
/// `ValueEnum` variant doc comments. `clap` produces these during its build phase, after the
/// [`mut_args`](clap::Command::mut_args) hook this module uses has already run, so localizing them
/// would require disabling `clap`'s automatic help/version handling and re-declaring it on every
/// command. That trade-off is not worth the added surface for a lightweight CLI; every
/// pinto-authored command and argument *description* is localized instead.
...
pub(super) enum Command {
    ...
    Doctor(DoctorArgs),
    /// Undo the most recent completed board mutation (git backend only).
    Undo,
...
#[cfg(test)]
mod localization_coverage_tests {
    use super::*;

    /// Collect the help text of every pinto-authored argument in the localized command tree.
    ///
    /// Keys are `"<path> <arg-id>"`. The clap built-in `help`/`version` flags and the generated
    /// `help` subcommand are skipped: clap adds them during its build phase and they are documented
    /// as out of scope for localization (see [`localized_command`]).
    fn collect_arg_help(command: &clap::Command, path: &str, out: &mut Vec<(String, String)>) {
        for arg in command.get_arguments() {
            if matches!(arg.get_id().as_str(), "help" | "version") {
                continue;
            }
            if let Some(help) = arg.get_help() {
                out.push((format!("{path} {}", arg.get_id()), help.to_string()));
            }
        }
        for subcommand in command.get_subcommands() {
            if subcommand.get_name() == "help" {
    ...
    /// Every argument help string must be translated for a non-English locale.
    ///
    /// A localized `--help` screen that leaves argument descriptions in English is more confusing
    /// than an untranslated one, because the translated headings imply the whole screen is
    /// localized. This test fails when any argument's Japanese help still matches its English
    /// fallback, which is exactly what a missing catalog key produces.
    #[test]
    fn japanese_help_has_no_english_fallback_arguments() {
        let english = pinto::i18n::localizer_from(Some("en-US"), None);
        let japanese = pinto::i18n::localizer_from(Some("ja-JP"), None);

        let mut english_help = Vec::new();
        collect_arg_help(&localized_command(&english), "pinto", &mut english_help);
        let mut japanese_help = Vec::new();
        collect_arg_help(&localized_command(&japanese), "pinto", &mut japanese_help);

        assert_eq!(
            english_help.len(),
            japanese_help.len(),
            "the localized command trees must expose the same arguments"
        );

        let untranslated: Vec<&str> = english_help
            .iter()
            .zip(&japanese_help)
            .filter(|((_, en), (_, ja))| en == ja)
            .map(|((path, _), _)| path.as_str())
            .collect();

        assert!(
            untranslated.is_empty(),
            "these arguments fall back to English in ja-JP help (add cli-arg-* catalog keys): {untranslated:?}"
        );
    }
}
