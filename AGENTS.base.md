...
This file provides instructions for coding agents and developers working in this
repository. See [`docs/DESIGN.md`](docs/DESIGN.md) for detailed design
decisions.
...
The task definitions in `mise.toml` wrap `cargo test --all-features`,
`cargo clippy --all-targets --all-features -- -D warnings`, Rust documentation,
`mdbook build`, and `cargo fmt --check`. Direct `cargo` commands are allowed,
but prefer `mise run` to keep local and CI behavior aligned.
...
which replaced `backlog.md` in T-19.**
...
- [`backlog.md`](backlog.md) is a **frozen** snapshot from the T-19 migration.
  Do not update it; keep it only for historical reference.
- See [`docs/migration.md`](docs/migration.md) for the migration procedure and
  background.
